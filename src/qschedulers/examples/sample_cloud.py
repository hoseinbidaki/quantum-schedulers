...
from qschedulers.schedulers import Scheduler, RoundRobinScheduler
from qschedulers.cloud.orchestrator import Orchestrator
...
if __name__ == "__main__":
    ...
    tasks = [
        QuantumTask(
            ...
            arrival_time=0,
        ),  # arrives while others still running
        QuantumTask(
            id=0,
            circuit=get_benchmark("qft", level=BenchmarkLevel.ALG, circuit_size=10),
            arrival_time=1,
        ),  # may fail on small backend
        QuantumTask(
            id=1,
            circuit=get_benchmark("ghz", level=BenchmarkLevel.ALG, circuit_size=5),
            arrival_time=0,
        ),  # shares arrival, must wait
        QuantumTask(
            id=2,
            circuit=get_benchmark("qft", level=BenchmarkLevel.ALG, circuit_size=15),
            arrival_time=0,
        ),  # likely too big for FakeHanoi
    ]
